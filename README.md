...
**開發者：Suckbob｜發行商：Monster_Ai_hk｜Guardian Ai 守護層內建**

> **【Monster AI v3.0.0 · 公測正式版】**  
> Monster AI 現已開放完整功能，無需等待其他版本。  
>  
> - 訪客無需登入即可使用全部功能  
> - 無限額 + 無審查 RP  
> - 支援角色創作與對話記憶  
> - 生成系統（圖像／影片）+ 語音 TTS  
> - 13 位寵物擬人 + AV 男優包（BG/BL 虛構）  
> - 括號（）演技：情感／動作／語氣  
>  
> **APK 下載：** [v3.0.0 Release](https://github.com/SB00001090/Monster_AI_Project/releases/tag/v3.0.0) · 直接檔案 [MonsterAI-3.0.0-guest.apk](https://github.com/SB00001090/Monster_AI_Project/releases/download/v3.0.0/MonsterAI-3.0.0-guest.apk)  
> （舊版： [v2.0.0-beta](https://github.com/SB00001090/Monster_AI_Project/releases/tag/v2.0.0-beta) · [v1.0.0-beta](https://github.com/SB00001090/Monster_AI_Project/releases/tag/v1.0.0-beta)）  
> 開發者：Suckbob｜發行商：Monster_Ai_hk

**本地優先、隱私保護** 的 AI 平台 — 角色扮演、多模態生成（圖像 + 影片 + TTS）、自癒、情緒自學、OC 反抄襲、加密訓練庫與 Guardian 守護，全部在你的電腦上執行。
...
- 自我修復，自動 LLM 備援
- Web UI（HTTP + WebSocket + React neon dashboard）— **公測正式版可用**
- 模組化架構 — 依需求啟用功能
...
| 功能 | 狀態 |
|------|------|
| **生成系統（圖像／影片）** | ✅ 已恢復／可用（ComfyUI + 品質篩選 + 歷史） |
| **語音系統（TTS）** | ✅ 已恢復／可用（Piper；XTTS 選用） |
| **訪客完整權限** | ✅ 可用（無需登入、可創作角色、無鎖定牆） |
| **13 位寵物擬人角色** | ✅ 可用（頭像 + 角色卡 + 對話範例 + 簡介） |
| **自癒系統** | ✅ 可用（對話逾時重試、LLM fallback、會話保留） |
| **情緒自學分析** | ✅ 可用（台灣 + 香港口語 → 對話注入） |
| **無審查 + 無限額** | ✅ 可用（本地 uncensored；僅未成年硬擋） |
| **Monster AI APK 公測正式版** | ✅ 可用 — [Releases v3.0.0](https://github.com/SB00001090/Monster_AI_Project/releases/tag/v3.0.0) |

### 平台功能

| 功能 | 狀態 |
|------|------|
| 聊天 + WebSocket / React UI | ✅ 可用 |
| SillyTavern 風格角色扮演（卡片、記憶、多會話） | ✅ 可用 |
| 自我修復 LLM + 生成重試 | ✅ 可用 |
| 圖像生成（ComfyUI + LoRA + LLM 提示詞） | ✅ 可用 |
| 防崩潰圖像品質篩選 + 自動重試 | ✅ 可用 |
| 文字轉影片（.mp4，自訂解析度/FPS） | ✅ 可用 |
| 提示詞輔助 / 防崩潰機制 | ✅ 可用 |
| 一鍵啟動（Monster AI + ComfyUI） | ✅ 可用 |
| 生成歷史（保留 30 天） | ✅ 可用 |
| 角色肖像 + 擬人頭像 | ✅ 可用 |
| 50 款 UI 主題 + 隱藏「藍玫瑰和籠子」 | ✅ 可用 |
| 手勢操作介面 | ✅ 可用 |
| 即時回報 / 自動更新檢查 | ✅ 可用 |
...
| 女角色 | 男角色 |
|--------|--------|
| 雪兒（薩摩耶）、小桃（柴犬）、布丁（柯基）、冰蓝（哈士奇） | 雷恩（德國牧羊犬）、影（杜賓）、陽光（金毛） |
| 棉花糖（博美）、奶油（比熊）、小圓（法國鬥牛犬） | 蒼牙（秋田）、疾風（邊境牧羊犬）、鐵爪（羅威納） |

角色檔：`data/characters/` · `data/characters/角色檔案包/` · Android assets 擬人半身頭像。
...
### React Web UI（公測正式版可用）

> **歷史備註：** 2026-07 曾暫緩強調 Web 優先開發；**現已轉向公測正式版**，Web／本機 dashboard 與 APK 同步可用，訪客完整權限、無限額。
...
| 項目 | 說明 |
|------|------|
| **定位** | **v3.0.0 公測正式版**（訪客 = 完整功能 · 無限額 · 無鎖定） |
| **開發者** | Suckbob |
| **發行商** | Monster_Ai_hk |
| **APK Release** | [v3.0.0](https://github.com/SB00001090/Monster_AI_Project/releases/tag/v3.0.0) |
| **原生 guest APK** | `apps/monster-ai-guest` · `MonsterAI-3.0.0-guest.apk` |
| **Capacitor 殼** | `apps/monster-ai-android` + 根目錄 `client/` |
| **舊版** | [v2.0.0-beta](https://github.com/SB00001090/Monster_AI_Project/releases/tag/v2.0.0-beta) · [v1.0.0-beta](https://github.com/SB00001090/Monster_AI_Project/releases/tag/v1.0.0-beta) |

上傳 APK：`scripts\upload-v3-release.ps1`（需 `gh auth login`）

## 路線圖

1. ~~Monster AI 公測正式版（訪客完整功能 + 無限額）~~ **已完成（v3.0.0）**
2. ~~生成系統／語音／13 角色 + AV 包／括號演技~~ **已完成**
3. 原生 AnimateDiff ComfyUI 工作流程（單次通過影片）強化
4. ~~Discord 機器人橋接~~（MonsterGuard 已推出）
5. LoRA 訓練啟動器（unsloth）
6. 第三方模組外掛 API
7. 發行管線：維護 **v3.0.0** Release 資產
...
**開發者：** Suckbob · **發行商：** Monster_Ai_hk · **定位：** Monster AI v2.0.0-beta（訪客免費公測）
